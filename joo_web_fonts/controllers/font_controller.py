...
class FontController(http.Controller):
    """
    Controller for retrieving the configured web font for the joo_web_fonts module.
    This endpoint is called by font_loader.js to dynamically apply the selected font
    to elements using the 'joo-font' CSS class.
    """

    @http.route('/joo_web_fonts/font', type='json', auth='user')
    def get_font(self):
        """
        ...
        return request.env['ir.config_parameter'].sudo().get_param(
            'joo_web_fonts.font',
            'dwpiexmincho'  # Default font key (matches default font-family defined in joo_web_fonts.css)
        )
